...
<body class="bg-gray-900 text-gray-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-gray-800 rounded-xl shadow-2xl p-8 space-y-6 border border-gray-700">
        <a href="." class="text-3xl font-extrabold text-center text-sky-400 tracking-tight"></a>
        ...
        @if(session('success'))
            <div class="bg-emerald-900/50 border border-emerald-700 text-emerald-300 p-4 rounded-lg text-base font-medium space-y-3">
                <p>{{ session('success') }}</p>
                <a href="login" class="inline-block px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-lg transition duration-300 ease-in-out">
                    Logar
                </a>
            </div>
        @endif
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input placeholder="Digite seu usuário..." id="user" type="text" name="user" value="{{ old('user', '') }}" required
            ...
            <div>
                ...
                <input placeholder="Digite seu e-mail..." id="email" type="email" name="email" value="{{ old('email', '') }}" required
                       class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-sky-500 focus:border-sky-500 placeholder-gray-400 text-gray-100 transition duration-200 ease-in-out">
                @error('email')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
