@php
// Código Laravel Blade aprimorado para a tela de registro de empresa.
...
<body class="bg-gray-900 text-gray-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-gray-800 rounded-xl shadow-2xl p-8 space-y-6 border border-gray-700 fade-in-up">
        ...
        <header class="flex items-center justify-center relative">
            <!-- Botão Voltar -->
            <a href="enterprise" class="absolute left-0 text-gray-400 hover:text-sky-400 transition duration-200">
        ...
        @if(session('success'))
            <div class="bg-emerald-900/50 border border-emerald-700 text-emerald-300 p-4 rounded-lg text-base font-medium space-y-3">
                <p>{{ session('success') }}</p>
                <a href="enterprise" class="inline-block px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-lg transition duration-300 ease-in-out shadow-md hover:shadow-lg">
                    Fazer Login
                </a>
            </div>
        @endif
        ...
        <form method="post" class="space-y-5">
            ...
            {{-- Name Input --}}
            <div>
                <label for="name" class="sr-only">Nome da Empresa</label>
                <input value="{{ old('name', '') }}" type="text" id="name" name="name" placeholder="Nome da Empresa..." required
            ...
            <div>
                ...
                <input value="{{ old('email', '') }}" type="email" id="email" placeholder="E-mail principal..." name="email" required
                       class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-sky-500 focus:border-sky-500 placeholder-gray-400 text-gray-100 transition duration-200 ease-in-out">
                @error('email')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input value="{{ old('phone', '') }}" type="text" id="phone" name="phone" placeholder="Número de telefone..."
                       class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-sky-500 focus:border-sky-500 placeholder-gray-400 text-gray-100 transition duration-200 ease-in-out">
                @error('phone')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="relative">
                ...
                <input type="password" id="password" name="password" placeholder="Definir senha..." required
